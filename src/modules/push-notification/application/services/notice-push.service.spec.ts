...
describe('NoticePushService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: ExpoNotificationService,
          useValue: mockExpoService,
        },
        {
          provide: ExpoPushTokenService,
  ...
  describe('sendNoticePush()', () => {
    it('존재하지 않는 공지 ID로 호출 시 NotFoundException을 던져야 한다', async () => {
      ...
      // 전송이나 예약이 호출되면 안 됨
      expect(mockExpoService.send).not.toHaveBeenCalled();
      expect(mockExpoService.reserve).not.toHaveBeenCalled();
    });
    ...
    it('예약 발송: reserveTime이 있으면 reserve()를 호출하고 send()는 호출하지 않아야 한다', async () => {
      ...
      expect(mockExpoService.reserve).toHaveBeenCalledWith(
        expect.objectContaining({
          title: '공지 제목',
          body: '공지 본문 내용입니다.',
          reserveTime: '2026-06-01T10:00:00.000Z',
