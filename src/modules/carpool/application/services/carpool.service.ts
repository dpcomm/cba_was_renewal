...
export class CarpoolService {
  ...
  async getAllCarpoolRooms(): Promise<CarpoolRoom[]> {
    const carpools = await this.carpoolRoomRepository
      ...
      .select(['carpool', 'driver.id', 'driver.name'])
  ...
  async getCarpoolRoomDetail(id: number): Promise<CarpoolDetailResponseDto> {
    ...
    return {
      ...
      members: carpool.members.map((m) => ({
        id: m.user.id,
        name: m.user.name,
        phone: m.user.phone, // TODO: 마스킹 임시 비활성화 - maskPhone(m.user.phone)
      })),
    };
  }
  ...
  async findAvailableCarpools(
    userId?: number,
  ): Promise<CarpoolWithDriverInfoResponseDto[]> {
    const qb = this.carpoolRoomRepository
      ...
      .select(['carpool', 'driver.id', 'driver.name', 'driver.phone'])

      .orderBy('carpool.departureTime', 'ASC');

    //  유저가 이미 참여 중인 카풀 제외
    if (userId != null) {
      qb.leftJoin('carpool.members', 'myMember', 'myMember.userId = :userId', {
        userId,
  ...
  async findParticipatingCarpools(
    ...
    const qb = this.carpoolRoomRepository
      ...
      // 참여 중인 카풀 필터 (필터용 조인)
      .innerJoin('carpool.members', 'myMember', 'myMember.userId = :userId', {
        userId,
      })
  ...
  async updateCarpoolStatus(
    dto: updateCarpoolstatusRequestDto,
  ): Promise<CarpoolRoom> {
  ...
  async checkCarpoolReady(currentTime: Date): Promise<void> {
    ...
    const readyCarpoolList = await this.carpoolRoomRepository
      ...
      .andWhere('carpool.departureTime < :targetTime', {
  ...
  async startCarpool(id: number): Promise<void> {
    ...
    // 카풀 상태 변경
    const carpool = await this.updateCarpoolStatus({
      roomId: id,
      newStatus: CarpoolStatus.In_Transit,
    });
