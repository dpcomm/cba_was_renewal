...
import {
  deleteExpoPushTokenRequestDto,
  registExpoPushTokenRequestDto,
} from '@modules/expo-push-token/application/dto/expo-push-token.request.dto';
import {
...
export class ExpoPushTokenController {
  ...
  async regist(@Body() dto: registExpoPushTokenRequestDto) {
  ...
  @ApiSuccessResponse({})
  async delete(@Body() dto: deleteExpoPushTokenRequestDto) {
    await this.expoPushTokenService.deleteToken(dto.token);
    return ok<null>(null, 'Success delete ExpoPushToken');
  }
}
