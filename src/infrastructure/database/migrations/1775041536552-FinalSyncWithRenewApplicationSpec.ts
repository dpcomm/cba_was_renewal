...
export class FinalSyncWithRenewApplicationSpec1775041536552 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const safeRenameColumn = async (
      ...
      try {
        const hasOldCol = await queryRunner.query(
          `SHOW COLUMNS FROM \`${table}\` LIKE '${oldCol}'`,
        );
        if (hasOldCol.length > 0) {
          await queryRunner.query(
            `ALTER TABLE \`${table}\` CHANGE \`${oldCol}\` \`${newCol}\` ${definition}`,
          );
        }
      } catch (e) {
        /* Ignore */
    ...
    // Retreat NULL 값 처리
    await queryRunner.query(
      `UPDATE \`Retreat\` SET \`location\` = '장소 미정' WHERE \`location\` IS NULL`,
    );
    // retreat_start_at 기반으로 NULL 값 채움
    const hasEndAtCol = await queryRunner.query(
      `SHOW COLUMNS FROM \`Retreat\` LIKE 'retreat_end_at'`,
    );
    if (hasEndAtCol.length > 0) {
      await queryRunner.query(
        `UPDATE \`Retreat\` SET \`retreat_end_at\` = \`retreat_start_at\` WHERE \`retreat_end_at\` IS NULL`,
      );
    } else {
      // camelCase 시절 컬럼이 남아있을 경우
      const hasCamelEndAt = await queryRunner.query(
        `SHOW COLUMNS FROM \`Retreat\` LIKE 'retreatEndAt'`,
      );
      if (hasCamelEndAt.length > 0) {
        await queryRunner.query(
          `UPDATE \`Retreat\` SET \`retreatEndAt\` = \`retreat_start_at\` WHERE \`retreatEndAt\` IS NULL`,
        );
      }
    }

    // Survey가 하나도 없는 경우 (현재 상태), 기본 설문 생성 (Application의 NOT NULL 제약조건을 위해 필수)
    const surveyCount = await queryRunner.query(
    ...
    const hasSurveyIdCamel = await queryRunner.query(
      `SHOW COLUMNS FROM \`Application\` LIKE 'surveyId'`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE \`Retreat\` CHANGE \`location\` \`location\` varchar(191) NOT NULL`,
    );
    await safeRenameColumn(
      'Retreat',
      'retreatEndAt',
    ...
    await queryRunner.query(
      `ALTER TABLE \`Retreat\` CHANGE \`retreat_end_at\` \`retreat_end_at\` datetime(3) NOT NULL`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE \`Application\` CHANGE \`survey_id\` \`survey_id\` int NOT NULL`,
    );
    ...
    const createIdx = async (
      ...
      if (!(await hasIdx(table, idx))) {
        await queryRunner.query(
          `CREATE ${unique ? 'UNIQUE' : ''} INDEX \`${idx}\` ON \`${table}\` (${cols})`,
        );
      }
    };
    ...
    await createIdx(
      'ApplicationMeal',
      'idx_application_meal_retreat_meal_id',
      '`retreat_meal_id`',
    );
    await createIdx(
      'ApplicationMeal',
      'ApplicationMeal_applicationId_retreatMealId_key',
      '`application_id`, `retreat_meal_id`',
      true,
    );
    await createIdx(
      'RetreatMeal',
      'RetreatMeal_retreatId_dayNumber_mealType_key',
      '`retreat_id`, `day_number`, `meal_type`',
      true,
    );
    await createIdx(
      'ApplicationTransport',
      'idx_application_transport_retreat_transport_id',
      '`retreat_transport_id`',
    );
    await createIdx(
      'ApplicationTransport',
      'ApplicationTransport_applicationId_direction_key',
      '`application_id`, `direction`',
      true,
    );
    await createIdx(
      'ApplicationTransport',
      'ApplicationTransport_applicationId_retreatTransportId_key',
      '`application_id`, `retreat_transport_id`',
      true,
    );
    await createIdx(
      'RetreatTransport',
      'RetreatTransport_retreatId_direction_type_name_key',
      '`retreat_id`, `direction`, `transport_type`, `name`',
      true,
    );
    await createIdx(
      'Answer',
      'idx_answer_application_question',
      '`application_id`, `question_id`',
    );
    await createIdx(
      'Answer',
      'idx_answer_question_option_id',
      '`question_option_id`',
    );
    await createIdx('Answer', 'idx_answer_question_id', '`question_id`');
    await createIdx(
      'Answer',
      'Answer_applicationId_questionId_questionOptionId_key',
      '`application_id`, `question_id`, `question_option_id`',
      true,
    );
    await createIdx(
      'QuestionOption',
      'QuestionOption_questionId_orderNo_key',
      '`question_id`, `order_no`',
      true,
    );
    await createIdx(
      'Question',
      'Question_surveyId_orderNo_key',
      '`survey_id`, `order_no`',
      true,
    );
    ...
    await createIdx(
      'Application',
      'idx_application_retreat_checkedin',
      '`retreat_id`, `checked_in_at`',
    );
    await createIdx(
      'Application',
      'idx_application_retreat_payment',
      '`retreat_id`, `payment_status`',
    );
    ...
    await createIdx(
      'Application',
      'idx_application_retreat_status',
      '`retreat_id`, `status`',
    );
    await createIdx(
      'Application',
      'Application_userId_retreatId_key',
      '`user_id`, `retreat_id`',
      true,
    );
    ...
    await safeAddFK(
      'ApplicationMeal',
      'ApplicationMeal_applicationId_fkey',
      'FOREIGN KEY (`application_id`) REFERENCES `Application`(`id`) ON DELETE CASCADE ON UPDATE CASCADE',
    );
    await safeAddFK(
      'ApplicationMeal',
      'ApplicationMeal_retreatMealId_fkey',
      'FOREIGN KEY (`retreat_meal_id`) REFERENCES `RetreatMeal`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE',
    );
    await safeAddFK(
      'RetreatMeal',
      'RetreatMeal_retreatId_fkey',
      'FOREIGN KEY (`retreat_id`) REFERENCES `Retreat`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE',
    );
    await safeAddFK(
      'ApplicationTransport',
      'ApplicationTransport_applicationId_fkey',
    ...
    await safeAddFK(
      'ApplicationTransport',
      'ApplicationTransport_retreatTransportId_fkey',
      'FOREIGN KEY (`retreat_transport_id`) REFERENCES `RetreatTransport`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE',
    );
    await safeAddFK(
      'RetreatTransport',
      'RetreatTransport_retreatId_fkey',
      'FOREIGN KEY (`retreat_id`) REFERENCES `Retreat`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE',
    );
    await safeAddFK(
      'Answer',
      'Answer_applicationId_fkey',
      'FOREIGN KEY (`application_id`) REFERENCES `Application`(`id`) ON DELETE CASCADE ON UPDATE CASCADE',
    );
    await safeAddFK(
      'Answer',
      'Answer_questionId_fkey',
      'FOREIGN KEY (`question_id`) REFERENCES `Question`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE',
    );
    await safeAddFK(
      'Answer',
      'Answer_questionOptionId_fkey',
      'FOREIGN KEY (`question_option_id`) REFERENCES `QuestionOption`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE',
    );
    await safeAddFK(
      'QuestionOption',
